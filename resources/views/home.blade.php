  ...
  <body>
    ...
    <div class="container">
        ...
        <main class="card-columns mt-3">
            ...
            <div class="card">
            <div class="card-body">
                  <p class="card-text">{{strtoupper($levelCourse->course->title)}}</p>
                </div>
                <div class="card-footer">
                  <small class="text-muted">
                    @if($levelCourse->hasExam())
                        <a href="{{route('exam',[$levelCourse->id])}}" class="btn examBtn">Start Exam {{$levelCourse->course->code}}</a>
                    @else
                       <div class="alert">No exam ready for {{$levelCourse->course->code}}</div>
                    @endif
                  </small>
                </div>
              </div>
